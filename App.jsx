...
const RATING_MIN = 2;
const RATING_MAX = 10;
// Fútbol 5: los goles no suman punto a punto (ver matchRating). En su lugar,
// el "Goleador del partido" y la "Mejor defensa del partido" (uno por equipo)
// reciben este reconocimiento fijo, sin importar cuántos goles se hicieron.
const F5_RECOGNITION_INC = 1;
// Media Competitiva: progresión de confianza según partidos jugados (estilo ranking ATP).
// Un jugador con pocos partidos todavía no "confirmó" su nivel real, así que su Media
// Competitiva se acerca gradualmente a su Promedio real a medida que suma partidos.
const MEDIA_COMPETITIVA_TABLE = { 1: 70, 2: 75, 3: 80, 4: 85, 5: 90, 6: 92, 8: 95 };
function mediaCompetitivaFactor(pj) {
  if (!pj || pj <= 0) return 0;
  if (pj >= 10) return 100;
  if (MEDIA_COMPETITIVA_TABLE[pj] != null) return MEDIA_COMPETITIVA_TABLE[pj];
  // Interpola linealmente entre los puntos conocidos de la tabla (cubre 7 y 9 partidos).
  const full = { ...MEDIA_COMPETITIVA_TABLE, 10: 100 };
  const known = Object.keys(full).map(Number).sort((a, b) => a - b);
  let lo = known[0], hi = known[known.length - 1];
  for (let i = 0; i < known.length - 1; i++) {
    if (pj > known[i] && pj < known[i + 1]) { lo = known[i]; hi = known[i + 1]; break; }
  }
  const t = (pj - lo) / (hi - lo);
  return full[lo] + t * (full[hi] - full[lo]);
}
// Media Competitiva = Promedio real ajustado por la confianza según partidos jugados.
// NO modifica estadísticas reales (goles, asistencias, premios, historial): solo se usa
// para ordenar el ranking.
function mediaCompetitiva(avgReal, pj) {
  const factor = mediaCompetitivaFactor(pj);
  return Math.round(avgReal * (factor / 100) * 100) / 100;
}
const QUICK_AWARDS = [
  "Mejor arquero",
  "Mejor defensa",
...
// Validaciones para evitar jugadores duplicados: se usan antes de crear o editar,
// tanto por nombre (ignorando mayúsculas/espacios) como por número de camiseta.
function normalizedPlayerName(name) {
  return (name || "").trim().toLowerCase().replace(/\s+/g, " ");
}
function playerNameTaken(players, name, excludeId) {
  const n = normalizedPlayerName(name);
  if (!n) return false;
  return players.some((p) => p.id !== excludeId && normalizedPlayerName(p.name) === n);
}
function playerNumberTaken(players, number, excludeId) {
  return players.some((p) => p.id !== excludeId && Number(p.number) === Number(number));
}
...
function matchRating(match, playerId, mvpVotes) {
  ...
  const isF5 = match.mode === "Fútbol 5";
  const f5 = match.f5Recognitions || {};
  const isF5Scorer = isF5 && (f5.scorerA === playerId || f5.scorerB === playerId);
  const isF5Defender = isF5 && (f5.defenderA === playerId || f5.defenderB === playerId);
  let rating = BASE_RATING;
  // En Fútbol 5 los goles no suman punto a punto: solo cuenta como reconocimiento
  // fijo (Goleador del partido), para que no pesen más que un buen partido general.
  if (!isF5) rating += goals * GOAL_INC;
  ...
  if (isF5Scorer) rating += F5_RECOGNITION_INC;
  if (isF5Defender) rating += F5_RECOGNITION_INC;
  rating -= ownGoals * OWN_GOAL_DEC;
  rating = Math.min(RATING_MAX, Math.max(RATING_MIN, rating));
  return { rating, goals, assists, ownGoals, votes, awards, win, cleanSheet, teamGoals, rivalGoals, isF5, isF5Scorer, isF5Defender };
}
...
// Tabla secundaria histórica: goleadores de Fútbol 5. NO es el ranking principal
// (ese se ordena por Media Competitiva); esto es solo un registro de goles convertidos
// en esa modalidad, ya que ahí el gol es un reconocimiento y no el factor determinante.
function computeF5Scorers(players, matches) {
  const map = {};
  matches.forEach((m) => {
    if (m.mode !== "Fútbol 5") return;
    m.participants.forEach((part) => {
      const g = Number(part.goals) || 0;
      if (g <= 0) return;
      if (!map[part.playerId]) map[part.playerId] = { goals: 0, matches: 0 };
      map[part.playerId].goals += g;
      map[part.playerId].matches += 1;
    });
  });
  return Object.entries(map)
    .map(([playerId, v]) => ({ p: players.find((p) => p.id === playerId), ...v }))
    .filter((x) => x.p)
...
function computeRankingMovement(players, matches, votes) {
  const buildOrder = (matchList) => {
    ...
    return players
      .map((p) => {
        const pj = map[p.id].pj;
        const avg = pj > 0 ? map[p.id].score / pj : 0;
        return { id: p.id, pj, avg, mc: mediaCompetitiva(avg, pj) };
      })
      .sort((a, b) => b.mc - a.mc || b.avg - a.avg || b.pj - a.pj);
  };
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const existingNumbers = new Set(loadedPlayers.map((p) => p.number));
      const missingPlayers = SEED_PLAYERS.filter((p) => !existingNumbers.has(p.number));
  ...
  const totals = useMemo(() => {
    const map = {};
    players.forEach((p) => (map[p.id] = { pj: 0, goals: 0, assists: 0, votes: 0, ownGoals: 0, score: 0, goalsW: 0, assistsW: 0, awards: 0 }));
    matches.forEach((m) => {
      const mv = votes[m.id];
      ...
      m.participants.forEach((p) => {
        ...
        if (r && map[p.playerId]) {
          ...
          // Premios: reconocimientos por categoría (awards) + Goleador/Mejor defensa de Fútbol 5.
          map[p.playerId].awards += r.awards.length + (r.isF5Scorer ? 1 : 0) + (r.isF5Defender ? 1 : 0);
        }
      });
    });
    ...
    () =>
      [...players]
        .map((p) => {
          ...
          const mc = mediaCompetitiva(avg, t.pj);
          return { ...p, ...t, avg, mediaCompetitiva: mc };
        })
        // El ranking principal ordena por Media Competitiva (no por el promedio real puro),
        // así un jugador con pocos partidos no salta al frente sin haber confirmado su nivel.
        .sort((a, b) => b.mediaCompetitiva - a.mediaCompetitiva || b.avg - a.avg || b.pj - a.pj),
    [players, totals]
  );
  ...
  const f5Scorers = useMemo(() => computeF5Scorers(players, matches), [players, matches]);
  ...
  async function addPlayer(name, position) {
    ...
    // Antes de crear, verificar que no exista ya un jugador con ese nombre
    // (evita duplicados por errores de tipeo, doble click, etc.).
    if (playerNameTaken(players, name, null)) {
      setError("Ya existe un jugador con ese nombre.");
      return;
    }
    const number = nextNumber(players);
    if (playerNumberTaken(players, number, null)) {
      setError("Ese número de camiseta ya está en uso.");
      return;
    }
    const p = { id: "p" + Date.now(), name: name.trim(), position, number };
  ...
  async function editPlayer(id, name, position) {
    ...
    if (!target) { setEditingPlayerId(null); return; }
    // Verificar duplicados excluyendo al propio jugador que se está editando,
    // y modificar SIEMPRE el documento existente (mismo id) — nunca crear uno nuevo.
    if (playerNameTaken(players, name, id)) {
  ...
  return (
    <div style={{ background: "#0a0c10", minHeight: "100vh", fontFamily: "'Rajdhani', system-ui, sans-serif" }} className="text-gray-200 pb-24">
      ...
      {showAddPlayer && isAdmin && (
        <AddPlayerModal
          onClose={() => { setShowAddPlayer(false); setError(""); }}
          onSave={addPlayer}
      ...
      {editingPlayerId && isAdmin && (
        <AddPlayerModal
          onClose={() => { setEditingPlayerId(null); setError(""); }}
      ...
      {showAddMatch && isAdmin && (
        <AddMatchModal onClose={() => { setShowAddMatch(false); setError(""); }} onSave={addMatch} players={players} error={error} />
      )}
      {editingMatchId && isAdmin && (
        <AddMatchModal
          onClose={() => { setEditingMatchId(null); setError(""); }}
...
function RankingTab({ ranking, leaders, matches, votes, movement, onOpenProfile }) {
  ...
  const valueFor = (p) => {
    ...
    return p.pj > 0 ? p.mediaCompetitiva.toFixed(1) : "—";
  };
    ...
    <div>
      ...
      <div className="rounded-2xl border border-gray-800 overflow-hidden" style={{ background: "rgba(255,255,255,0.02)" }}>
        <div className="grid grid-cols-12 text-[10px] text-gray-500 px-3 py-2 bg-gray-900/70 disp tracking-wider uppercase">
          ...
          <div className="col-span-3 text-right">{quickTab === "general" ? "M. Competitiva" : quickTab === "vallas" ? "Vallas" : quickTab === "goleadores" ? "Goles" : "Asist."}</div>
        </div>
        ...
        {rest.map((p, i) => (
          ...
          >
            ...
            <div className="col-span-3 flex flex-col items-end">
              ...
              {quickTab === "general" && p.pj > 0 && (
                <span className="text-[9px] text-gray-600 mt-0.5">prom. real {p.avg.toFixed(1)}</span>
              )}
            </div>
          </button>
        ))}
...
function AddPlayerModal({ onClose, onSave, count, initial, error }) {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    // onSave (addPlayer/editPlayer) valida duplicados y decide si cierra el modal;
    // el guard de "submitting" evita que un doble click cree/edite dos veces.
    await onSave(name, position);
    setSubmitting(false);
  }
...
function AddMatchModal({ onClose, onSave, players, initial, error }) {
  const isEdit = !!initial;
  ...
  const [f5, setF5] = useState({
    scorerA: initial?.f5Recognitions?.scorerA || "",
    scorerB: initial?.f5Recognitions?.scorerB || "",
    defenderA: initial?.f5Recognitions?.defenderA || "",
    defenderB: initial?.f5Recognitions?.defenderB || "",
  });
  const [submitting, setSubmitting] = useState(false);
  const isF5 = mode === "Fútbol 5";
  ...
  async function submit() {
    ...
    setSubmitting(true);
    // Si quedó una categoría seleccionada (label + jugadores) sin apretar "Agregar
    // reconocimiento", la sumamos igual acá para que nunca se pierda al guardar.
    const pendingAward =
      awardLabel.trim() && awardPlayers.length > 0
        ? awardPlayers.map((pid) => ({ label: awardLabel.trim(), playerId: pid }))
        : [];
    const finalAwards = [...awards, ...pendingAward];
    const teamRows = (team) => rows.filter((r) => r.team === team);
    const finalF5 = isF5
      ? {
          scorerA: teamRows("A").some((r) => r.playerId === f5.scorerA) ? f5.scorerA : "",
          scorerB: teamRows("B").some((r) => r.playerId === f5.scorerB) ? f5.scorerB : "",
          defenderA: teamRows("A").some((r) => r.playerId === f5.defenderA) ? f5.defenderA : "",
          defenderB: teamRows("B").some((r) => r.playerId === f5.defenderB) ? f5.defenderB : "",
        }
      : undefined;
    await onSave({ date, mode, teamAName, teamBName, participants: rows, awards: finalAwards, f5Recognitions: finalF5 });
    setSubmitting(false);
  }
        ...
        {isF5 && (
          <div className="border-t border-gray-900 pt-3">
            <span className="text-[11px] text-gray-500">
              Reconocimientos de Fútbol 5 (no suman por cantidad de goles, cada uno vale +1 punto)
            </span>
            <div className="grid grid-cols-2 gap-2 mt-2">
              <div>
                <label className="text-[10px] text-gray-500">Goleador · {teamAName}</label>
                <select
                  value={f5.scorerA}
                  onChange={(e) => setF5((s) => ({ ...s, scorerA: e.target.value }))}
                  className="w-full mt-1 bg-gray-950 border border-gray-800 rounded-lg px-2 py-1.5 text-xs text-gray-200"
                >
                  <option value="">—</option>
                  {rows.filter((r) => r.team === "A").map((r) => {
                    const pl = players.find((p) => p.id === r.playerId);
                    return pl ? <option key={pl.id} value={pl.id}>{pl.name}</option> : null;
                  })}
                </select>
              </div>
              <div>
                <label className="text-[10px] text-gray-500">Goleador · {teamBName}</label>
                <select
                  value={f5.scorerB}
                  onChange={(e) => setF5((s) => ({ ...s, scorerB: e.target.value }))}
                  className="w-full mt-1 bg-gray-950 border border-gray-800 rounded-lg px-2 py-1.5 text-xs text-gray-200"
                >
                  <option value="">—</option>
                  {rows.filter((r) => r.team === "B").map((r) => {
                    const pl = players.find((p) => p.id === r.playerId);
                    return pl ? <option key={pl.id} value={pl.id}>{pl.name}</option> : null;
                  })}
                </select>
              ...
              <div>
                <label className="text-[10px] text-gray-500">Mejor defensa · {teamAName}</label>
                <select
                  value={f5.defenderA}
                  onChange={(e) => setF5((s) => ({ ...s, defenderA: e.target.value }))}
                  className="w-full mt-1 bg-gray-950 border border-gray-800 rounded-lg px-2 py-1.5 text-xs text-gray-200"
                >
                  <option value="">—</option>
                  {rows.filter((r) => r.team === "A").map((r) => {
              ...
              <div>
                <label className="text-[10px] text-gray-500">Mejor defensa · {teamBName}</label>
                <select
                  value={f5.defenderB}
                  onChange={(e) => setF5((s) => ({ ...s, defenderB: e.target.value }))}
                  className="w-full mt-1 bg-gray-950 border border-gray-800 rounded-lg px-2 py-1.5 text-xs text-gray-200"
                >
                  <option value="">—</option>
                  {rows.filter((r) => r.team === "B").map((r) => {
            ...
            <p className="text-[9px] text-gray-600 mt-1">
              Los goles siguen contando para la tabla histórica de Goleadores Fútbol 5, pero no definen el ranking principal.
            </p>
          </div>
        )}
  ...
  return (
    <div className="rounded-2xl border border-gray-800 overflow-hidden" style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.035), rgba(255,255,255,0.01))" }}>
      ...
      <div className="px-4 pb-4">
        ...
        {m.mode === "Fútbol 5" && m.f5Recognitions && (m.f5Recognitions.scorerA || m.f5Recognitions.scorerB || m.f5Recognitions.defenderA || m.f5Recognitions.defenderB) && (
          <div className="flex flex-wrap gap-1.5 mb-2.5">
            {[
              ["Goleador del partido", m.f5Recognitions.scorerA],
              ["Goleador del partido", m.f5Recognitions.scorerB],
              ["Mejor defensa del partido", m.f5Recognitions.defenderA],
              ["Mejor defensa del partido", m.f5Recognitions.defenderB],
            ].map(([label, pid], i) => {
              if (!pid) return null;
              ...
              return (
                <span key={i} className="text-[10px] bg-gray-900/70 border border-gray-800 text-gray-400 rounded-full px-2 py-0.5">
                  {label}: <span style={{ color: "#00C853" }}>{pl.name}</span>
                </span>
              );
            })}
          </div>
...
function EstadisticasTab({ leaders, idealTeam, f5Scorers, onOpenProfile }) {
  const {
    topScorer, topAssist, mostMVP, mostMatches, highestAvg, bestKeeper, cleanSheetsMap, breakout,
    mostOffensive, bestSingleGoal, bestSingleAssist, bestStreak, mostAchievements,
  } = leaders;
  ...
  return (
    <div>
      ...
      {f5Scorers && f5Scorers.length > 0 && (
        <>
          <h3 className="disp text-white text-lg tracking-wide mb-1 flex items-center gap-2"><Goal size={16} style={{ color: "#0D6EFD" }} /> Goleadores Fútbol 5</h3>
          <p className="text-[10px] text-gray-600 mb-2">
            Tabla histórica de goles en Fútbol 5. No es el ranking principal: ahí el gol suma como reconocimiento, no punto por punto.
          </p>
          <div className="flex flex-col gap-1.5 mb-6">
            {f5Scorers.slice(0, 10).map((x, i) => (
              <button
                key={x.p.id}
                onClick={() => onOpenProfile(x.p.id)}
                className="flex items-center justify-between rounded-xl border border-gray-800 px-3 py-2 text-left"
                style={{ background: "rgba(255,255,255,0.02)" }}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-[10px] text-gray-600 w-4 shrink-0">{i + 1}</span>
                  <span className="jersey shrink-0" style={{ width: 26, height: 26, fontSize: 11 }}>{x.p.number}</span>
                  <span className="text-xs text-gray-200 truncate">{x.p.name}</span>
                </div>
                <span className="text-xs disp shrink-0" style={{ color: "#0D6EFD" }}>{x.goals} goles</span>
              </button>
            ))}
          </div>
        </>
    ...
    <div className="relative rounded-[24px] p-4 border-2 overflow-hidden" style={{ background: meta.grad, borderColor: meta.ring, boxShadow: `0 16px 40px -16px ${meta.ring}88` }}>
      ...
      <div className="relative flex items-start justify-between">
        <div>
          ...
          <div className="text-[10px] text-gray-300 disp -mt-1">Promedio real · {player.position}</div>
          {t.pj > 0 && <div className="text-[10px] text-gray-400 mt-0.5">Media Competitiva: <span className="disp" style={{ color: meta.ring }}>{mc.toFixed(2)}</span></div>}
        </div>
      ...
      <div className="relative grid grid-cols-5 gap-1.5 mt-3 pt-3 border-t" style={{ borderColor: meta.ring + "33" }}>
        {[["PJ", t.pj], ["⚽ Goles", t.goals], ["🎯 Asist.", t.assists], ["🗳️ MVP", t.votes], ["🏅 Premios", t.awards ?? 0]].map(([label, val]) => (
